  ...
  switch (spec.response) {
    ...
    case 'selected_iids': {
      ...
      if (need.kind === 'choose_units') {
        ...
        const selected = [...new Set(response.selected_iids || [])].filter(iid => eligible.has(iid));
        if (selected.length > (need.take_up_to ?? Infinity))
          throw new Error(`Cannot select more than ${need.take_up_to} units`);
        const zone = need.zone || spec.zone || 'field';
        const bindings = [];
      ...
      if (need.scry_trash) {
        for (const ref of kept) b[bindPid].zones.trash.push(ref);
        b[bindPid].zones.deck.unshift(...rest);
        break;
      }
