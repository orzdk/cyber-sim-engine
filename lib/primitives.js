...
function spendTapped(p, amount) {
  ...
  const stillReady = p.tapped.filter(iid =>
    p.zones.eddies.some(e => e.iid === iid && e.state === 'ready') ||
    p.zones.legends.some(l => l.iid === iid && l.state === 'ready'));
  if (stillReady.length < amount)
    throw new Error(`Need ${amount} tapped resource(s) — currently ${stillReady.length} tapped`);
  const toSpend = stillReady.slice(0, amount);
  const spentLegends = [];
