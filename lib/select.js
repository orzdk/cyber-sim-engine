...
function selectTarget(target, b, ctx, bind) {
  ...
  if (quantifier === 'upto_n') {
    ...
    if (target.chooser === 'controller' && n > 0 && pool.length > n) {
      ...
      const kindMulti = (target.type === 'Unit' && zone === 'field') || (target.type === 'Legend' && zone === 'legends')
        ? 'choose_units' : null;
      if (kindMulti) {
        return {
          kind: 'halt',
  ...
  const askAnyway = target.optional && target.side === 'both';
  if (chooser === 'auto' || (pool.length === 1 && !askAnyway)) {
    const sorted = _sortAuto(pool, target, ctx);
